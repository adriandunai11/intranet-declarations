...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Nyilatkozatcsomag #<?= (int) $packet->id ?></h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <?php if ($person): ?>
                        <li class="breadcrumb-item">
                            <a href="<?= url('declarations/persons/' . $person->id) ?>"><?= esc($person->fullName()) ?></a>
                        </li>
                    <?php endif; ?>
                    <li class="breadcrumb-item active">Csomag #<?= (int) $packet->id ?></li>
                </ol>
            </div>
        </div>
    </div>
...
<section class="content">
    <div class="row">
        ...
        <div class="col-lg-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <?php if (empty($editableTemplates ?? [])): ?>
                            <p class="text-muted mb-0">Nincs további hozzáadható online kitölthető nyilatkozat.</p>
                        <?php else: ?>
                            ...
                            <select name="template_id" id="add_template_id" class="form-control mr-2 mb-2" required>
                                ...
                                <?php foreach ($editableTemplates as $template): ?>
                                    <option value="<?= (int) $template->id ?>"><?= esc($template->displayName()) ?></option>
                                <?php endforeach; ?>
                            </select>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Nyilatkozatok a csomagban</h3>
                    <?php if (!empty($batchRejectItems ?? [])): ?>
                        <div class="card-tools">
                            <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="modal" data-target="#batchRejectItemsModal">
                ...
                <div class="card-body">
                    <?php if (empty($reviewItems)): ?>
                        <p class="text-muted mb-0">A csomagban nincs nyilatkozat.</p>
                    <?php else: ?>
                            ...
                            <?php foreach ($reviewItems as $index => $reviewItem): ?>
                                ...
                                <div class="card mb-2">
                                    <div class="card-header p-0" id="<?= esc($headingId) ?>">
                                        <button class="btn btn-link btn-block text-left text-decoration-none p-3" type="button" data-toggle="collapse" data-target="#<?= esc($collapseId) ?>" aria-expanded="<?= $isOpen ? 'true' : 'false' ?>" aria-controls="<?= esc($collapseId) ?>">
                                            <div class="d-flex justify-content-between align-items-start flex-wrap">
                                                ...
                                                <div class="text-right">
                                                    <span class="badge badge-<?= esc($itemStatusClass) ?>"><?= esc($itemStatusLabel) ?></span>
                                                    <?php if ($submission && !empty($submission->submitted_at)): ?>
                                                        <div class="text-muted small mt-1">Beküldve: <?= esc($submission->submitted_at) ?></div>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </button>
                                    </div>
                                    <div id="<?= esc($collapseId) ?>" class="collapse <?= $isOpen ? 'show' : '' ?>" aria-labelledby="<?= esc($headingId) ?>" data-parent="#packetReviewAccordion">
                                        <div class="card-body">
                                            <?php if (!$submission): ?>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <p class="mb-2"><span class="badge badge-<?= esc($invitationStatusClass) ?>"><?= esc($invitationStatusLabel) ?></span></p>
                        </div>
                        <div class="col-md-6">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="sendNewInvitationLinkModalLabel">Új meghívó link küldése</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Bezárás">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>Biztosan meghívó linket szeretne generálni és kiküldeni a kitöltőnek?</p>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-warning">Generálás és küldés</button>
...
<?php if (!empty($reviewItems)): ?>
    <?php foreach ($reviewItems as $reviewItem): ?>
        ...
        <?php if ($submission && $item->status === 'completed' && $canReview): ?>
            <div class="modal fade" id="rejectItemModal<?= (int) $item->id ?>" role="dialog" data-backdrop="static" aria-labelledby="rejectItemModalLabel<?= (int) $item->id ?>" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <?= form_open('declarations/packets/' . $packet->id . '/items/' . $item->id . '/reject') ?>
                        <?= csrf_field() ?>
                        <div class="modal-header">
                            <h5 class="modal-title" id="rejectItemModalLabel<?= (int) $item->id ?>">Nyilatkozat elutasítása</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Bezárás">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <p>Biztosan elutasítod ezt a nyilatkozatot?</p>
                            <p class="text-muted"><?= esc($item->template_name ?: '-') ?></p>
                            <div class="form-group">
                                <label for="review_note_<?= (int) $item->id ?>" class="required">Javítás oka / megjegyzés</label>
                        ...
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-danger"><i class="fas fa-times pr-1"></i> Elutasítás</button>
                            <button type="button" class="btn btn-default" data-dismiss="modal">Mégsem</button>
                        </div>
                        <?= form_close() ?>
                    </div>
            ...
            <div class="modal fade" id="reopenItemForCorrectionModal<?= (int) $item->id ?>" tabindex="-1" role="dialog" aria-labelledby="reopenItemForCorrectionModalLabel<?= (int) $item->id ?>" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <?= csrf_field() ?>
                        <div class="modal-header">
                            <h5 class="modal-title" id="reopenItemForCorrectionModalLabel<?= (int) $item->id ?>">Nyilatkozat újranyitása javításra</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Bezárás">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <p class="text-muted">Ez az elem újra kitölthető lesz a meghívó linken. Az elfogadás/elutasítás adatai törlődnek.</p>
                            <div class="form-group">
                    ...
                    <?php foreach ($batchRejectItems as $reviewItem): ?>
                        ...
                        <div class="border rounded p-3 mb-3">
                            <div class="custom-control custom-checkbox mb-2">
                                ...
                                <label class="custom-control-label" for="batch_reject_item_<?= (int) $item->id ?>"><?= esc($item->template_name ?: ('Nyilatkozat #' . $item->id)) ?></label>
                            </div>
